...
const TRANSLATIONS = {
  en: {
    ...
    sec3Title: "Sprite Player & Editor",
    sec3Sub: "Powerful, easy-to-use playback and editing tools",
    sec3Desc: "Advanced features including background removal, pixel-perfect frame editing, and smart export options.",
    feat1: "Background Removal",
    feat2: "Pixel Editing",
    feat3: "Sprite Sheet Export",
    galleryTitle: "GALLERY",
    gallerySub: "Generated by Rika AI",
  ...
  zh: {
    ...
    tutorialTitle: "观看教程",
    sec3Title: "像素编辑器",
    sec3Sub: "强大且易用的播放与编辑工具",
    sec3Desc: "包含自动背景移除、像素级帧编辑和序列帧导出等高级功能。",
    feat1: "背景移除",
    feat2: "像素编辑",
    feat3: "序列帧导出",
    galleryTitle: "画廊",
    gallerySub: "由 Rika AI 生成",
    faqTitle: "常见问题",
    ...
    faqs: [
      ...
      {
        q: "生成的动画是像素完美的吗？",
        a: "是的，Rika AI 生成的动画保证像素完美，没有模糊的像素或抗锯齿伪影。"
      },
      {
        q: "生成一次动画需要多少积分？",
        a: "标准生成每次消耗 5 个积分。如果生成任务失败，积分将自动退还到你的账户中。"
      },
      {
        q: "如果我对生成的结果不满意怎么办？",
        a: "AI 生成的序列并不总是能一次成功。你可以在dashboard中选择重新生成来重新加载参数，调整提示词或动作类型，然后再次尝试。"
      },
      {
        q: "生成的动画可以用于我的游戏项目吗？",
        a: "当然可以！Rika AI 支持免费导出序列帧和动图，这些资源可以直接导入任何游戏引擎。"
      }
    ]
  }
};
...
const LandingPage: React.FC<LandingPageProps> = ({ onGetStarted, onViewDocs, lang = 'en' }) => {
  ...
  return (
    <div className={`bg-[#0d0221] text-white selection:bg-[#5a2d9c] selection:text-white overflow-hidden font-pixel ${isZh ? 'zh-text' : ''}`}>
      {/* SECTION 1: HERO */}
      <section className="min-h-[80vh] flex flex-col lg:flex-row items-center justify-center px-6 lg:px-24 gap-12 py-16">
        <div className="flex-1 space-y-6 text-center lg:text-left">
          ...
          <div className="space-y-4">
             <h2 className={`text-base md:text-lg font-medium text-white/90 leading-tight uppercase tracking-wider text-pretty ${isZh ? 'text-[11px]' : ''}`}>
                {t.heroSub}
             </h2>
             <p className={`uppercase tracking-[0.15em] leading-relaxed max-w-md mx-auto lg:mx-0 text-pretty ${isZh ? 'text-[14px] text-white/40' : 'text-[8px] md:text-[9px] text-white/40'}`}>
                {t.heroDesc}
                <br /><br />
                <span className={'text-[8px] md:text-[9px] text-white/40'}>{t.heroFoot}</span>
             </p>
          </div>
          <div className="pt-4">
            <PixelButton 
      ...
      <section className="min-h-screen flex flex-col justify-center py-24 px-6 lg:px-24 bg-black/10 border-t border-[#5a2d9c]/10">
        <div className="text-center mb-24 space-y-2">
          <h2 className="text-xl md:text-2xl font-bold uppercase tracking-tight text-[#f7d51d] text-pretty">{t.sec2Title}</h2>
          <p className={`uppercase tracking-[0.4em] text-pretty ${isZh ? 'text-[11px] text-white/50' : 'text-[8px] text-white/30'}`}>{t.sec2Sub}</p>
        </div>
        ...
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-start justify-center gap-12 lg:gap-0 w-full px-4 relative">
          <div className="flex-1 flex flex-col items-center gap-6 w-full relative">
            ...
            <h3 className={`font-bold uppercase text-white tracking-widest text-center whitespace-nowrap ${isZh ? 'text-[11px]' : 'text-[7.5px] md:text-[8px]'}`}>
              {t.step1}
            </h3>
            ...
            <div className="text-white text-[9px] uppercase opacity-40 tracking-[0.3em] h-8 flex items-center">64x64</div>
          </div>
        ...
        <div className="max-w-4xl mx-auto w-full mt-28 px-4 flex flex-col items-center">
          <div className="mb-6 flex flex-col items-center gap-1">
            <h3 className={`font-bold uppercase tracking-[0.3em] text-[#f7d51d] ${isZh ? 'text-[11px]' : 'text-[10px] md:text-[11px]'}`}>{t.tutorialTitle}</h3>
      ...
      <section className="min-h-[70vh] flex flex-col justify-center py-24 px-6 lg:px-24 bg-[#0d0221]">
        <div className="text-center mb-20 space-y-2">
          <h2 className="text-xl md:text-2xl font-bold uppercase tracking-tight text-[#f7d51d] text-pretty">{t.sec3Title}</h2>
          <p className={`uppercase tracking-[0.4em] text-pretty ${isZh ? 'text-[11px] text-white/50' : 'text-[8px] text-white/30'}`}>{t.sec3Sub}</p>
          <p className={`pt-4 text-white/40 uppercase tracking-[0.2em] max-w-2xl mx-auto text-pretty ${isZh ? 'text-[10px]' : 'text-[9px] md:text-[10px]'}`}>
            {t.sec3Desc}
          </p>
        </div>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6 w-full items-stretch px-4">
          {[
            { title: t.feat1, img: `${CDN_BASE}bgremove.gif` },
            { title: t.feat2, img: `${CDN_BASE}edit.gif` },
            { title: t.feat3, img: `${CDN_BASE}export.gif` }
          ].map((feature) => (
            <div key={feature.title} className="flex flex-col bg-[#1e1e1e] border-2 border-[#3a3a3a] shadow-[8px_8px_0px_#111] overflow-hidden group">
              <div className="flex items-center justify-between px-4 py-3 bg-[#3a3a3a] border-b-2 border-[#4a4a4a]">
                <span className={`font-bold uppercase tracking-wider text-white/90 ${isZh ? 'text-[11px]' : 'text-[10px]'}`}>{feature.title}</span>
      ...
      <section className="py-24 bg-black/20 border-t border-[#5a2d9c]/10 relative overflow-hidden">
        <div className="text-center mb-16 space-y-2 px-6">
          <h2 className="text-xl md:text-2xl font-bold uppercase tracking-tight text-[#f7d51d] text-pretty">{t.galleryTitle}</h2>
          <p className={`uppercase tracking-[0.4em] text-pretty ${isZh ? 'text-[11px] text-white/50' : 'text-[8px] text-white/30'}`}>{t.gallerySub}</p>
        </div>
      ...
      <section className="py-24 px-6 lg:px-24 bg-[#0d0221] border-t border-[#5a2d9c]/10">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-16 space-y-2">
            <h2 className="text-xl md:text-2xl font-bold uppercase tracking-tight text-[#f7d51d] text-pretty">{t.faqTitle}</h2>
            <p className={`uppercase tracking-[0.4em] text-pretty ${isZh ? 'text-[11px] text-white/50' : 'text-[8px] text-white/30'}`}>{t.faqSub}</p>
          </div>
            ...
            {t.faqs.map((item, idx) => (
              <div key={idx} className="space-y-4 text-left border-l-2 border-[#5a2d9c]/30 pl-6 hover:border-[#f7d51d] transition-colors">
                <div className="flex items-start gap-4">
                  ...
                  <h3 className={`font-bold uppercase leading-relaxed text-white text-pretty ${isZh ? 'text-[14px]' : 'text-[10px] md:text-[11px]'}`}>
                    {item.q}
                  </h3>
                </div>
                <div className="flex items-start gap-4">
                  <span className="text-white/40 font-bold text-xs shrink-0">A:</span>
                  <p className={`leading-relaxed text-white/60 text-pretty ${isZh ? 'text-[13px]' : 'text-[9px] md:text-[10px]'}`}>
                    {item.a}
                  </p>
                </div>
              </div>
      ...
      <footer className="pt-24 pb-12 text-center bg-[#0d0221] flex flex-col items-center border-t border-[#5a2d9c]/10">
         <div className="max-w-2xl mx-auto space-y-6 px-6 mb-16">
            <div className="flex flex-col items-center justify-center space-y-2">
              <h3 className="text-lg font-bold uppercase tracking-tight text-white text-pretty">{t.ctaTitle}</h3>
              <p className={`uppercase tracking-[0.3em] text-pretty text-[10px] text-white/30`}>{t.ctaSub}</p>
            </div>
            <div className="pt-4">
